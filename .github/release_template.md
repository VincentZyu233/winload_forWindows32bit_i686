...
| **i686 (32-bit)** | [![basic](https://img.shields.io/badge/basic-i686-555555.svg?logo=windows)](__BASE_URL__/winload-windows-i686-nonpcap-__VERSION__.exe) · [![legacy](https://img.shields.io/badge/legacy-i686-969696.svg?logo=windows)](__BASE_URL__/winload-windows-i686-legacy-__VERSION__.exe) | [![binary](https://img.shields.io/badge/binary-i686-E95420.svg?logo=linux)](__BASE_URL__/winload-linux-i686-__VERSION__) | *Not provided* | *Not provided* |

> **Labels**: `npcap` = with Npcap loopback capture (requires Npcap installed, Windows 7+); `basic` = standalone MSVC binary, no Npcap (Windows 7+); `legacy` = MinGW-w64 for legacy Windows (XP/Vista/7+).<br>
> **Toolchain**: `npcap` & `basic` are built with MSVC (VS 2022, requires Windows 7+ with KB2999226). `legacy` variants use MinGW-w64 — runs on Windows XP through 11, pick this if the MSVC binary fails to start on your system.<br>
